...
- Request inputs use Valibot schemas via h3 validated helpers: `readValidatedBody` for request bodies, `getValidatedRouterParams` for route params, and `getValidatedQuery` for query strings. Schemas live in `server/utils/validation/schemas.ts`.
...
- For single-row `DELETE` operations that need the deleted fields for response/logging, prefer `delete(...).where(...).returning(...)` over a separate pre-read plus delete.
