...
test.describe('Gear library item actions', () => {
  ...
  test('should render saved status from the catalog without loading all My Gear rows', async ({
    context,
    page
  }) => {
    ...
    const tracker = await mockCatalogApi(context, {
      items: () => {
        return {
          json: {
            ...firstPageResponse,
            items: [savedStoveItem, sleepingPadItem]
          }
        }
      }
    })
  ...
  test('should keep a My Gear removal over a stale catalog snapshot', async ({
    context,
    page
  }) => {
